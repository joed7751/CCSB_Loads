...
plot(pTHg_Inletm1,ann=FALSE)
title(main = "11452600_pTHg Response vs Fitted Values",xlab = "Fitted Values",ylab = "Response Values")
plot(pTHg_Inletm1,which = 2,set.up = F)
title(main = "11452600_pTHg Residuals vs Fitted Values")
plot(pTHg_Inletm1,which = 3,set.up = F)
title(main = "11452600_pTHg Assessing Heteroscedasticity") #Add "of Residuals"?
plot(pTHg_Inletm1,which = 4,set.up = F)
title(main = "11452600_pTHg Correlogram of Samples")
plot(pTHg_Inletm1,which = 5,set.up = F)
title(main="11452600_pTHg Normal Discharge")
plot(pTHg_Inletm1,which = 6,set.up = F)
title(main="11452600_pTHg Box Plot of Loads")

#These functions plot the data using the chosen best model and add a title and labels to the plot.

pTHg_Inlet_load<-predLoad(pTHg_Inletm1,InletQ,load.units="kg",by="water year",allow.incomplete = TRUE,conf.int = 0.95,print = TRUE)
write.csv(pTHg_Inlet_load,"2_Inlet_pTHg_m1_Flux_Annual.csv")
pTHg_Inlet_load_day<-predLoad(pTHg_Inletm1, InletQ,load.units = "kg",by="day",allow.incomplete = TRUE,conf.int = 0.90,print = TRUE)
write.csv(pTHg_Inlet_load_day,"2_Inlet_pTHg_m1_Flux_Daily.csv")

...
pTHg_South <- importRDB("South_pTHg_2010-2019.txt")
pTHg_South
...
setSweave("pTHg_South_lr2_plot2",5,5)
plot(pTHg_South_lr2,which=1,set.up=FALSE) #plot 1: Calibrated regression model
graphics.off()

setSweave("pTHg_South_lr2_plot1",5,5)
plot(pTHg_South_lr2,which=2,set.up=FALSE) #plot 2:Estimates of log load versus model residuals from AMLE regression
graphics.off()

setSweave("pTHg_South_lr2_plot3",5,5)
plot(pTHg_South_lr2,which=3,set.up=FALSE)# plot 3: Estimates of log load versus squared model residuals 
graphics.off()

setSweave("pTHg_South_lr2_plot4",5,5)
plot(pTHg_South_lr2,which=4,set.up=FALSE) # plot 4: Standardized Serial Correlation
graphics.off()

setSweave("pTHg_South_lr2_plot5",5,5)
plot(pTHg_South_lr2,which=5,set.up=FALSE) # plot 5: Normal probability plot for the model residuals (Quantile plot)
graphics.off()

#############################################################################################
#predict concntrations
###############################################################################################
preds_conc_lr2 <- predConc(pTHg_South_lr2, SouthQ, by= "day", allow.incomplete= TRUE )
preds_conc_lr2
exportCSV(preds_conc_lr2, file.name = "South_pTHg_conc_lr2_2010-2019.cvs")
...
preds_Flux_lr2day<-predLoad(pTHg_South_lr2,SouthQ,load.units="kg",by="day",allow.incomplete = TRUE,conf.int = 0.95,print = TRUE)
preds_Flux_lr2day
exportCSV(preds_Flux_lr2day, file.name = "South_pTHgFlux_lr2_Day2010-2019.cvs")


preds_Flux_lr2WY<-predLoad(pTHg_South_lr2,SouthQ,load.units="kg",by="water year",allow.incomplete = TRUE,conf.int = 0.95,print = TRUE)
preds_Flux_lr2WY
exportCSV(preds_Flux_lr2WY, file.name = "South_pTHg_Flux_lr2_WY2010-2019.cvs")
